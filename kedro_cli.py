...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/my_dummy_example")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=my_dummy_example"]
    )
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "my_dummy_example"
